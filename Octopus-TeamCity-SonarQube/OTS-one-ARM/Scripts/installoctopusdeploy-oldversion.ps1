...
$a = (get-date -Format hh-m-s) + "." +  (Get-Random) + ".octopusdeploy.log"
...
$octopusDeployVersion = "Octopus.2018.3.8-x64"
$msiFileName = "Octopus.2018.3.8-x64.msi"
...
Stop-Transcript
Write-Host "-OctopusURI:" $OctopusURI   "*****"   "-OctopusUsername:"  $OctopusUsername   "****"   "-OctopusPassword:"  "*****" $OctopusPassword | Out-File -Append C:\filename.txt
Get-ChildItem -Force C:\Scripts | Out-File -Append C:\filename.txt
$a = (get-date -Format hh-m-s) + "." +  (Get-Random) + ".scriptexecute.log"
Start-Transcript -Path "C:\$a"
. "C:/Scripts/octopus-api-token-create.ps1" -OctopusURI $OctopusURI -OctopusUsername $OctopusUsername -OctopusPassword $OctopusPassword 2>&1 > D:\Install-API-Tentacle.ps1.log
Stop-Transcript
#$a = (get-date -Format hh-m-s) + "." +  (Get-Random) + ".certificatescriptexecute.log"
#Start-Transcript -Path "C:\$a"
. "C:/Scripts/certconfigureoctohttps.ps1" -OctopusURI $OctopusURI  2>&1 > D:\installCert-ConfigureOctopusHttps.ps1.log
#Stop-Transcript
