...
Param(
    ...
    [string] $ResourceGroupName = 'dcom-ci-cd',
...
if ($UploadArtifacts) {
    ...
    if ($StorageAccount -eq $null) {
        $StorageResourceGroupName = 'OTS_Staging'
...
$JSON = Get-Content $TemplateParametersFile | Out-String | ConvertFrom-Json
$linuxVMName = $JSON.parameters.virtualMachineName.value
$sonaradmin = $JSON.parameters.adminUsername.value
$sqFQDN = ("{0}.{1}.{2}" -f $JSON.parameters.sonarDnsName.value, $ResourceGroupLocation, 'cloudapp.azure.com')
